...
- **Watch-only wallet** — Descriptor tab → **Wallet**: the current policy as a watch-only wallet (no signing). Refresh derives receive/change via Core and lists coins via Electrum/Fulcrum. Balance, used addresses, UTXOs.
...
- **Watch-only-Wallet** — Descriptor-Tab → **Wallet**: die aktuelle Policy als Beobachtungs-Wallet (kein Signing). Aktualisieren leitet Empfang/Wechsel über Core ab und listet Coins über Electrum/Fulcrum. Saldo, benutzte Adressen, UTXOs.
