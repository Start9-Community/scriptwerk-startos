...
- **Watch-only wallet** — Descriptor tab → **Wallet**: the current policy as a watch-only wallet (no signing). With Core connected, descriptor check and UTXO scan run on load. Refresh derives receive/change via Core and lists coins via Electrum/Fulcrum. Balance, used addresses, per-address UTXOs with sum. Units: BTC / sats / Auto (below 0.01 BTC as sats).
